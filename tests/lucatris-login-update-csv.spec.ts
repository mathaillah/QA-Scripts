...
// Function to update CSV line with Lucatris Login status
function updateCsvLine(line: string, lucatrisStatus: string): string {
  const parts = line.split(';');
  // Ensure we have at least 7 columns (index 6 is Lucatris)
  while (parts.length < 8) {
    parts.push('');
  }
  parts[6] = lucatrisStatus; // Update Lucatris column (index 6)
  return parts.join(';');
}

test('Lucatris Login test and update CSV', async ({ page, context }) => {
  ...
  console.log(`\n========================================`);
  console.log(`Lucatris Login Test & CSV Update`);
  ...
  for (const rowIndex of successRowIndices) {
    if (updatedLines[rowIndex]) {
      ...
      console.log(`  Updated row ${rowIndex}: Lucatris Login = YES`);
    }
  }
  ...
  const resultsFile = path.join(resultsDir, `lucatris-login-csv-update-${timestamp}.md`);

  let mdContent = `# Lucatris Login Test Results (CSV Update)\n\n`;
  ...
  mdContent += `| Successful (Lucatris Login = YES) | ${successUsers.length} |\n`;
